...
```{r finding common peaks in all samples}

# run find_common_peaks function 
common_peaks <- suppressWarnings(find_common_peaks(my_peaks))
print("here are the number of peaks that overlap in all samples")
length(common_peaks)

#TODO insert screen shot of good coomon peak
```
...
print("This is how many peaks are common in non-dox")
...
print("This is how many peaks are common in dox")
...
print("This is how many peaks are common in both non- and dox treatments")
...
print("This is how many peaks are unique to non-dox condition")
length(unique_to_non_dox)

# Peaks unique to dox
unique_to_dox <- find_my_peaks(dox_non_dox_ov, dox_common_peaks)

print("This is how many peaks are unique to dox condition")
...
```{r creating genome annotation GRanges}
...
```{r dox and non-dox unique peak overalp gene promoters}
...
print("This is how many dox_unique peaks overlapped gene promoters")
length(dox_gene_promoter_ov)
...
print("this is how many genes overlap between RNAseq and ATACseq non-dox peaks")
length(sig_rnaseq_atac_non_dox)

print("Here are the gene names that overlap")
print(sig_rnaseq_atac_non_dox$gene_name)

# filter significant genes from RNAseq "filtered_res_df" to non-dox unique promoter overlaps
sig_rnaseq_atac_dox <- non_dox_gene_promoter_ov[dox_gene_promoter_ov$gene_id %in% filtered_res_df$gene_id]

print("this is how many genes overlap between RNAseq and ATACseq non-dox peaks")
length(sig_rnaseq_atac_dox)
...
peaks were changing in dox and non-dox conditions. 
...
```{r create input files and run DESEQ2 on feature counts of consensus peaks from NF_CORE}
...
```{r analysis of DESEQ2 on ATAC peak counts}
